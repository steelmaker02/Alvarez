...
export const ProductCarousel: React.FC = () => {
  ...
  // Функция переключения коллекций
  const switchCollection = (direction: 'next' | 'prev') => {
    // Запускаем анимацию исчезновения
    setIsAnimating(true);

    setTimeout(() => {
      // Меняем коллекцию через 300мс (когда контент исчез)
    ...
    <section id="neuheiten" className="py-24 bg-[#F5F0EB] text-[#2B1B17] relative overflow-hidden min-h-[700px]">
      ...
      <div className="container mx-auto px-6 md:px-12 mb-12 flex flex-col items-center text-center">
        ...
        <div className={`mb-8 transition-opacity duration-300 ${isAnimating ? 'opacity-0' : 'opacity-100'}`}>
          {/* ИСПРАВЛЕНО: text-[#9D6B48] (темнее для контраста) */}
          <span className="text-[#9D6B48] font-sans text-sm tracking-[0.2em] uppercase mb-3 block">
        ...
        <div className="flex space-x-6">
          <button
            ...
            aria-label="Vorherige Kollektion" // Добавил для доступности (Accessibility)
          ...
          <div className="flex items-center gap-2">
            {collections.map((_, idx) => (
              <div
                key={idx}
                // ИСПРАВЛЕНО: bg-[#9D6B48] для активной точки
                className={`w-2 h-2 rounded-full transition-all duration-300 ${idx === activeCollectionIndex ? 'bg-[#9D6B48] w-4' : 'bg-[#2B1B17]/20'}`}
              ></div>
            ))}
          </div>
          ...
          <button
            ...
            aria-label="Nächste Kollektion" // Добавил для доступности
            className="w-12 h-12 rounded-full border border-[#2B1B17]/20 flex items-center justify-center hover:bg-[#2B1B17] hover:text-[#F5F0EB] transition-all duration-300"
          >
            <ArrowRight size={20} />
          </button>
...
const ProductCard: React.FC<{ product: Product }> = ({ product }) => {
  ...
  return (
    <div className="relative group w-full sm:w-[calc(50%-1rem)] lg:w-[calc(20%-2rem)] min-w-[240px] max-w-[300px] flex flex-col">
      {/* ИСПРАВЛЕНО: цвета фона и рамки под новый оттенок #9D6B48 */}
      <div className="relative aspect-[4/5] bg-[#9D6B48]/10 rounded-sm overflow-hidden mb-5 shadow-sm border border-[#9D6B48]/20">
        ...
        <div className="absolute inset-x-0 bottom-0 p-4 opacity-0 translate-y-4 transition-all duration-500 ease-out group-hover:opacity-100 group-hover:translate-y-0 flex justify-end">
          <button
            onClick={handleAdd}
            aria-label="In den Warenkorb" // Добавил для Accessibility
            // ИСПРАВЛЕНО: hover:bg-[#9D6B48]
            className="w-12 h-12 bg-[#F5F0EB] text-[#2B1B17] rounded-full flex items-center justify-center shadow-lg hover:bg-[#9D6B48] hover:text-white transition-colors duration-300"
          >
            <Plus size={22} />
          </button>
        </div>
      ...
      <div className="text-left pr-0">
        ...
        <div className="mt-4 pt-3 border-t border-[#2B1B17]/10 flex justify-between items-baseline">
          {/* ИСПРАВЛЕНО: text-[#9D6B48] - теперь цена более читабельна */}
          <span className="font-sans text-[#9D6B48] font-medium text-lg md:text-xl">{product.price}</span>

          {/* ИСПРАВЛЕНО: text-[#2B1B17]/75 - мелкий текст стал четче */}
