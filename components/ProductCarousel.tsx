    ...
    <section id="neuheiten" className="py-24 bg-[#F5F0EB] text-[#2B1B17] relative overflow-hidden min-h-[700px]">
      ...
      <div className="container mx-auto px-6 md:px-12 mb-12 flex flex-col items-center text-center">
        ...
        <div className={`mb-8 transition-opacity duration-300 ${isAnimating ? 'opacity-0' : 'opacity-100'}`}>
          <span className="text-[#C68E66] font-sans text-sm tracking-[0.2em] uppercase mb-3 block">
        ...
        <div className="flex space-x-6">
          ...
          <div className="flex items-center gap-2">
            {collections.map((_, idx) => (
              <div
                key={idx}
                className={`w-2 h-2 rounded-full transition-all duration-300 ${idx === activeCollectionIndex ? 'bg-[#C68E66] w-4' : 'bg-[#2B1B17]/20'}`}
              ></div>
            ))}
          </div>
      ...
      {/* Сетка товаров (Grid вместо Slider, чтобы все 5 красиво встали по центру) */}
...
const ProductCard: React.FC<{ product: Product }> = ({ product }) => {
  ...
  return (
    // Увеличил min-w до 280px на больших экранах, чтобы текст влезал
    <div className="relative group w-full sm:w-[calc(50%-1rem)] lg:w-[calc(20%-2rem)] min-w-[240px] max-w-[300px] flex flex-col">
      <div className="relative aspect-[4/5] bg-[#C68E66]/10 rounded-sm overflow-hidden mb-5 shadow-sm border border-[#C68E66]/20">
        ...
        {/* Кнопка + (Чуть увеличил размер 10 -> 12 для удобства клика) */}
        <div className="absolute inset-x-0 bottom-0 p-4 opacity-0 translate-y-4 transition-all duration-500 ease-out group-hover:opacity-100 group-hover:translate-y-0 flex justify-end">
          <button
            onClick={handleAdd}
            className="w-12 h-12 bg-[#F5F0EB] text-[#2B1B17] rounded-full flex items-center justify-center shadow-lg hover:bg-[#C68E66] hover:text-white transition-colors duration-300"
          >
            <Plus size={22} />
          </button>
        </div>
      ...
      <div className="text-left pr-0">
        {/* Заголовок: text-2xl (24px) - отлично для чтения */}
        <h3 className="font-serif text-xl md:text-2xl text-[#2B1B17] mb-2 leading-tight">{product.name}</h3>

        {/* Описание: text-sm md:text-base (16px) - теперь это читается легко! */}
        {/* text-[#2B1B17]/80 - чуть темнее для контраста */}
        ...
        <div className="mt-4 pt-3 border-t border-[#2B1B17]/10 flex justify-between items-baseline">
          {/* Цена: text-xl (20px) - крупнее */}
          <span className="font-sans text-[#C68E66] font-medium text-lg md:text-xl">{product.price}</span>
          <span className="text-xs text-[#2B1B17]/50 font-normal uppercase tracking-wider">/ Stück</span>
        </div>
      </div>
    </div>
  );
